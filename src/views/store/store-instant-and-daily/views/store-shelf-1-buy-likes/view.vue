<template>
  <div id="store-shelf-1-buy" class="store-shelf-1-buy spring">
    ...
    <div class="wrapper">
        ...
        <div class="tab-content">
          <div class="store-container">
            ...
            <transition name="fade-tabs" mode="out-in">
                    ...
                    <div class="pc">
                      ...
                      >
                        ...
                        <span v-if="pkg.payment_type === 1" class="coins">
                          ...
                          <sub class="del">{{ pkg['original_price'] | numToFixed }} coins</sub>
                        </span>
                        <span v-if="pkg.payment_type === 2" class="coins">
                          <sup v-if="pkg['product_type'] === 2">
                            <b>
                              {{ $t('global.currencySymbol') }}
                              {{ pkg.price_decimal / pkg.purchase_quantity * 100 | numToFixed }}
                            </b>
                            / 100 {{ $t('global.instagramConcept.followersLowercase') }}
                          </sup>
                          <sup v-if="pkg['product_type'] === 1">
                            <b>
                              {{ $t('global.currencySymbol') }}
                              {{ pkg.price_decimal / pkg.purchase_quantity * 100 | numToFixed }}
                            </b>
                            / 100 {{ $t('global.instagramConcept.likesLowercase') }}
                          </sup>
                          <sub>
                            Total:
              ...
              <div v-if="tabsIndex" key="box1" class="pkg-container">
                ...
                <transition name="fade-skeleton" mode="out-in">
                  <div v-if="!productPkgListLoading">
                    <div class="pc">
                      <div v-for="(pkg, i) in productPkgListLike"
                           :key="i"
                           :class="{
                             'follow': pkg['product_type'] === 2,
                             'like': pkg['product_type'] === 1,
            ...
            <transition-group name="fade-tabs" mode="out-in">
              <!--post-container-->
              <div v-if="insUser.ins_id && tabsIndex" :key="1" class="post-container">
      ...
      <!--圣诞活动-->
      <div class="spring__santa-claus" :class="{ hide: santaClausHide }"></div>
    </div>
...
export default {
  ...
  computed: {
    productPkgListDisplay: function () {
      ...
      console.log(84841,this.productPkgList)
  ...
  methods: {
    ...
    renderProductPkgList(pkgList) {
      ...
      this.renderPkyListDaily(pkgList);
    },
    renderPkgListHighlight() {
      console.log('productPkgListFollow',this.productPkgListFollow)
      let indexFollow = 0;
    ...
    renderPkyListDaily(pkgList) {
      ...
      //   product_type: 2,
      //   promote_sale_type: 1,
      //   purchase_quantity: 1110
      // });


      pkgList.map(item => {
        if (item['cycle_type'] > 1) {
          item.dailyQuantity = item['purchase_quantity'] / item['cycle_type'];
          this.pkgListWithUnit.push(item);
        }
      });
      const res = new Map();
      this.productPkgListDaily = this.pkgListWithUnit.filter((a) => !res.has(a.dailyQuantity) && res.set(a.dailyQuantity, 1));
    ...
    bottomBtnDetective() {
      if (!this.tabsIndex) {
        ...
        this.bottomBtnOn = this.productPkgListLikeIndex !== -1 && this.postListIndex !== -1 && this.insUser.ins_id;
      }
    ...
    addToCart() {
      ...
      if (this.tabsIndex) { // 点赞
        ...
        param.price_decimal = this.productPkgCurrentLike.price_decimal || 0;
